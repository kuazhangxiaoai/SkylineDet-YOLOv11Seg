...
model = torch.hub.load("ultralytics/yolov5", "custom", path="path/to/yolov11n-sky-seg.pt")  # local model
model = torch.hub.load("path/to/yolov5", "custom", path="path/to/yolov11n-sky-seg.pt", source="local")  # local repo
```
