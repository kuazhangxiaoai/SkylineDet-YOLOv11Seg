...
from ultralytics.utils.metrics import (SegmentMetrics,SkySegmentMetrics, box_iou, mask_iou, line_eval, SkylineMetrics,
                                       mask_mcr, dice_score, mask_accuracy, mask_recall, mask_precision)
from ultralytics.utils.plotting import (output_to_target, plot_images,
                                        plot_eval_images_skyseg, get_edge, plot_eval_images_skyline, plot_masks)
from ultralytics.utils.torch_utils import de_parallel
from ultralytics.data import build_sky_dataset, build_dataloader

class SkySegmentationValidator(DetectionValidator):
    """
    ...
    def __init__(self, dataloader=None, save_dir=None, pbar=None, args=None, _callbacks=None):
        ...
        self.metrics = SkySegmentMetrics(save_dir=self.save_dir, on_plot=self.on_plot)
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, pred in enumerate(preds):
            ...
            stat = dict(
                ...
                dice_score=torch.zeros(1, 1, dtype=torch.float, device=self.device),
                mcr=torch.zeros(1, 1, dtype=torch.float, device=self.device)
            )
...
class SkyLineValidator(DetectionValidator):
    ...
    def __init__(self, dataloader=None, save_dir=None, pbar=None, args=None, _callbacks=None):
        ...
        self.process = None
        self.args.task = "skyseg"
        self.metrics = SkylineMetrics(save_dir=self.save_dir, on_plot=self.on_plot)
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, (pred, proto) in enumerate(zip(preds[0], preds[1])):
            ...
            stat = dict(
                ...
                mu=torch.zeros(1, 1, dtype=torch.float, device=self.device),
                sigma=torch.zeros(1, 1, dtype=torch.float, device=self.device),
                min=torch.zeros(1, 1, dtype=torch.float, device=self.device),
                max=torch.zeros(1, 1, dtype=torch.float, device=self.device),
            )
            pbatch = self._prepare_batch(si, batch)
            cls, bbox = pbatch.pop("cls"), pbatch.pop("bbox")
            nl = len(cls)
            ...
            if nl:
                ...
                _, pred_edge,_ = get_edge((pred_semantic_masks[si]>0.5).float(), batch['im_file'][si], plot=False)
                _, gt_edge,_ = get_edge((gt_semantic_masks[si] > 0.5).float(), batch['im_file'][si], plot=False)

                stat['mu'] = line_eval(pred_edge, gt_edge)
                stat['mu'] = torch.from_numpy(stat['mu'][None])
                #stat['sigma'] = torch.from_numpy(stat['sigma'][None])
                #stat['min'] = torch.from_numpy(stat['min'][None])
                #stat['max'] = torch.from_numpy(stat['max'][None])
                if self.args.plots:
            ...
            if self.args.plots:
                plot_eval_images_skyline(pred_masks[si],
                                         gt_semantic_masks[si],
                                         pred_edge,
                                         gt_edge,
                                         self.save_dir,
                                         batch['im_file'][si],
...
def validate(cfg=DEFAULT_CFG):
    ...
    validator = SkySegmentationValidator(val_dataloader, Path(cfg.name))
    #validator = SkyLineValidator(val_dataloader, Path(cfg.name))
    validator(model=model)


if __name__ == '__main__':
    validate(cfg=SKYSEG_CFG)