...
from ultralytics.utils import DEFAULT_CFG, RANK, RSISEG_CFG
from ultralytics.utils.plotting import plot_images, plot_results
from ultralytics.utils.torch_utils import de_parallel
from ultralytics.data import build_potsam_dataset
from ultralytics.utils.plotting import plot_masks

class RSISegmentationTrainer(yolo.detect.DetectionTrainer):
    """
    ...
    def get_validator(self):
        ...
        self.loss_names = "loss"
        return yolo.rsiseg.RSISegmentationValidator(
...
def train(cfg=DEFAULT_CFG, use_python=False):
    ...
    args = dict(model=model, data=data, device=device, task="rsiseg")
    if use_python:
        from ultralytics import YOLO
        YOLO(model).train(**args)
    else:
        trainer = RSISegmentationTrainer(cfg=cfg, overrides=args)
        trainer.train()


if __name__ == '__main__':
    train(cfg=RSISEG_CFG)