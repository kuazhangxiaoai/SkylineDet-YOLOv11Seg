    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment,SkySegment, Pose, OBB)) else self.forward(x)
...
class SkySegmentationModel(DetectionModel):
...
def torch_safe_load(weight, safe_only=False):
    ...
    ckpt, file = torch_safe_load("path/to/yolov11n-sky-seg.pt", safe_only=True)
    ```
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    if "backbone" in d.keys() and "head" in d.keys():
        modules = d["backbone"] + d["head"]
    else:
        modules = d["encoder"] + d["neck"] + d["decoder"]
    if scales:
        scale = d.get("scale")
        if not scale:
...
def yaml_model_load(path):
    ...
    d["scale"] = d["scale"] or guess_model_scale(path)
