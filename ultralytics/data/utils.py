...
def get_boundingbox_from_mask(mask, format="xyxy"):
    ...
    for c in contours:
        ...
        if area > 10:
            ...
            if format == "xywh":
                bboxes.append([x+0.5*w, y+0.5*h, w, h])
            elif format == "xyxy":
                bboxes.append([
                    x,
                    y,
...
def verify_sky_image_and_mask_2(args):
    ...
    try:
        ...
        #verify mask
        name = im_file.split(os.sep)[-1]
        mask_name = "Mask_" + name
        mask = cv2.imread(lb_file.replace(name, mask_name))
        msk_shape = mask.shape
        assert msk_shape == img_shape

        segments = mask2polygon(mask)
        ...
        lb[:, 3] = lb[:, 3] - lb[:, 1]  # to [xmin, ymin, w, h] (normalized => 1.0)
        lb[:, 4] = lb[:, 4] - lb[:, 2]
        return im_file, lb, img_shape, segments, keypoints, nm, nf, ne, nc, msg
    except Exception as e:
        nc = 1
        msg = f"{prefix}WARNING ⚠️ {im_file}: ignoring corrupt image/label: {e}"
