...
from ultralytics.data.utils import mask2polygon
...
def semantic_transforms(dataset, imgsz, hyp, stretch=False):
    """
    Applies a series of image transformations for training.
    ...
    return Compose(
        [
            ...
            Albumentations(p=1.0),
            RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),
            SemanticRandomFlip(direction="vertical", p=hyp.flipud),
            SemanticRandomFlip(direction="horizontal", p=hyp.fliplr, flip_idx=flip_idx),
        ]
    )  # transforms
