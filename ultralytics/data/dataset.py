...
from .utils import (
    ...
    verify_image,
    verify_image_label,
    verify_sky_image_and_mask,
    verify_rsi_image_and_mask
)
...
class SkyDataset(BaseDataset):
    ...
    def __init__(self, *args, data=None, task="skyseg",skyline=False, **kwargs):
        """Initializes the YOLODataset with optional configurations for segments and keypoints."""
        self.use_segments = task in ["segment","skyseg"]
    ...
    def img2label_paths(self, im_files):
        return[imgfile.replace('images', 'labels') for imgfile in im_files]
...
class PotsamDataset(BaseDataset):
    """
        Dataset class for loading object detection and/or segmentation labels in YOLO format.
    ...
    def __init__(self, *args, data=None, task="rsiseg", **kwargs):
        """Initializes the YOLODataset with optional configurations for segments and keypoints."""
        self.use_segments = task in ["segment", "skyseg","rsiseg"]
        self.use_keypoints = task == "pose"
        self.use_obb = task == "obb"
        self.data = data
        assert not (self.use_segments and self.use_keypoints), "Can not use both segments and keypoints."
    ...
    def img2label_paths(self, im_files):
        return [imgfile.replace('images', 'labels').replace("RGB","label") for imgfile in im_files]
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        #for im_file, label_file in zip(self.im_files, self.label_files):
         #   args = (im_file, label_file, self.data['colors'], self.prefix, self.use_keypoints, len(self.data['names']), nkpt, ndim)
         #   results.append(verify_rsi_image_and_mask(args=args))
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(
                func=verify_rsi_image_and_mask,
                iterable=zip(
                    self.im_files,
                    self.label_files,
    ...
    def split_mask(self, mask):
        ...
        h, w, c = mask.shape
        results = np.zeros((h, w, nc), dtype=np.float_)
    ...
    def update_labels_info(self, label):
        ...
        bbox_format = label.pop("bbox_format")
        normalized = label.pop("normalized")
        mask = self.split_mask(label.pop("mask"))
        # NOTE: do NOT resample oriented boxes
        segment_resamples = 100 if self.use_obb else 1000
        if len(segments) > 0:
            # list[np.array(1000, 2)] * num_samples
