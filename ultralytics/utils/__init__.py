...
SKYSEG_CFG_PATH = ROOT / "cfg/skyseg.yaml"
RSISEG_CFG_PATH = ROOT / "cfg/rsiseg.yaml"
...
SKYSEG_CFG_DICT = yaml_load(SKYSEG_CFG_PATH)
for k, v in SKYSEG_CFG_DICT.items():
    if isinstance(v, str) and v.lower() == "none":
        SKYSEG_CFG_DICT[k] = None
SKYSEG_CFG_DICT_KEYS = SKYSEG_CFG_DICT.keys()
SKYSEG_CFG = IterableSimpleNamespace(**SKYSEG_CFG_DICT)

# rsiseg configuration
RSISEG_CFG_DICT = yaml_load(RSISEG_CFG_PATH)
for k, v in RSISEG_CFG_DICT.items():
    if isinstance(v, str) and v.lower() == "none":
        RSISEG_CFG_DICT[k] = None
RSISEG_CFG_DICT_KEYS = RSISEG_CFG_DICT.keys()
RSISEG_CFG = IterableSimpleNamespace(**RSISEG_CFG_DICT)
...
def threaded(func):
    ...
    Use as @threaded decorator. The function exp in a separate thread unless 'threaded=False' is passed.
    """
...
class SettingsManager(JSONDict):
    ...
    Examples:
        ...
        >>> settings.update(runs_dir="/new/exp/dir")
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        self.defaults = {
            ...
            "runs_dir": str(root / "exp"),  # Experiment exp directory
...
RUNS_DIR = Path(SETTINGS["runs_dir"])  # global exp directory
ENVIRONMENT = (
    "Colab"
    if IS_COLAB
    else "Kaggle"
