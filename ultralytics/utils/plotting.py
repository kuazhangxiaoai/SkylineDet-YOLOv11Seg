...
def remapping_letter_box(img, orih, oriw, center=True, fillhole=False):
    h, w = img.shape
    r = min(h / orih, w / oriw)
    new_unpad = int(round(orih * r)), int(round(oriw * r))
    dh, dw = h - new_unpad[0], w - new_unpad[1]
    if center:
        dh, dw = round(dh / 2), round(dw / 2)
    img_np = (img.clone().cpu().numpy() * 255)[dh: h - dh, dw: w - dw]
    img_np = cv2.resize(img_np.astype(np.uint8), dsize=(oriw, orih), interpolation=cv2.INTER_CUBIC)
    if fillhole:
        contours, hierarchy = cv2.findContours(img_np, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        image = np.zeros_like(img_np)
        max_area = 0
        for c in contours:
        ...
        for i, contour in enumerate(contours):
            ...
            if area > 0.25 * max_area:
                image = cv2.drawContours(image, contours, i, color=(255), thickness=cv2.FILLED)
    else:
        image = img_np
...
def tensor2numpy(image):
    return image.clone().cpu().numpy() * 255

def get_edge(mask, img_path, plot=False, save_dir=None, use_ori_scale=False):
    h, w = mask.shape
    img = cv2.imread(img_path)
    orih, oriw = img.shape[:-1] if use_ori_scale else (h, w)
    mask = remapping_letter_box(mask, orih, oriw) if use_ori_scale else tensor2numpy(mask)

    mask_pred_down = 255 * np.ones((orih + 1, oriw), np.uint8)
    mask_pred_unsq = 255 * np.ones((orih + 1, oriw), np.uint8)
    mask_pred_down[1: ,:] = mask
    mask_pred_unsq[:-1,:] = mask
    mask_pred_down[0, :] = 255
    mask_pred_unsq[-1, :] = 0
    edge = mask_pred_down - mask_pred_unsq
    edge_index_y, edge_index_x = np.where(edge > 125)
    edge_arr = np.stack([edge_index_x, edge_index_y], axis=-1)
    name = img_path.split(os.sep)[-1]
    suffix = '.' + name.split('.')[-1]

    if plot:
        for point in edge_arr:
            cv2.circle(img, center=point, radius=1, color=(0,0,255), thickness=1)
    if save_dir is not None:
        cv2.imwrite(os.path.join(save_dir, name.replace(suffix, '_edge'+suffix)), img)
    return edge[:-1, :], np.array(edge_arr), img

def letter_box(image, imgsz=(640,640), scaleup=False, center=True):
    shape = image.shape[:-1]
...
def plot_eval_images_skyline(pred_masks, true_masks, pred_edge, true_edge, save_dir, img_path, center=True, draw_gt=False):
    h, w = pred_masks.shape
    visual_img = cv2.imread(img_path)
    visual_img, top, left, bottom, right = letter_box(image=visual_img, imgsz=(h, w))
    orih, oriw = visual_img.shape[:-1]
    r = min(h / orih, w / oriw)
    new_unpad = int(round(orih * r)), int(round(oriw * r))
    dh, dw = h - new_unpad[0], w - new_unpad[1]
    if center:
    ...
    name = img_path.split(os.sep)[-1]
    m, n = pred_edge.shape[0], true_edge.shape[0]
    for i in range(m):
        p = pred_edge.tolist()[i]
        x, y = p[0], p[1]
        cv2.circle(visual_img, center=[x, y], radius=1, color=[0, 255, 255], thickness=1)
    if draw_gt:
        for i in range(n):
            p = true_edge.tolist()[i]
            x, y = p[0], p[1]
            cv2.circle(visual_img, center=[x, y], radius=1, color=[0, 0, 255], thickness=1)
    visual_img = visual_img[top: h-bottom, left:w-right,:]
    cv2.imwrite(os.path.join(save_dir, name.replace('.jpg', '_edge.jpg')), visual_img)

def plot_eval_images_skyseg(pred_masks, true_masks, save_dir, img_path, center=True):
    #_,_,edge = get_edge(pred_masks, img_path, plot=True)
    h, w = pred_masks.shape
    orih, oriw = cv2.imread(img_path).shape[:-1]
    r = min(h / orih, w / oriw)
    new_unpad = int(round(orih * r)), int(round(oriw * r))
    dh, dw = h - new_unpad[0], w - new_unpad[1]
    if center:
        dh, dw = round(dh / 2), round(dw / 2)
    mask_true = (true_masks.cpu().numpy() * 255)[dh: h - dh, dw: w - dw]
    mask_pred = (pred_masks.cpu().numpy() * 255)[dh: h - dh, dw: w - dw]
    mask_true = cv2.resize(mask_true.astype(np.uint8), dsize=(oriw, orih), interpolation=cv2.INTER_CUBIC)
    mask_pred = cv2.resize(mask_pred.astype(np.uint8), dsize=(oriw, orih), interpolation=cv2.INTER_CUBIC)
    visual_img = np.zeros((orih, oriw, 3), np.uint8)
    name = img_path.split(os.sep)[-1]
    for i in range(orih):
        for j in range(oriw):
            pixel_true, pixel_pred = mask_true[i, j], mask_pred[i, j]
            if pixel_true > 125 and pixel_pred > 125:  # tp
                visual_img[i, j, 0] = 255
                visual_img[i, j, 1] = 0
                visual_img[i, j, 2] = 0
            elif pixel_pred > 125 and pixel_true < 125:  # fp
                visual_img[i, j, 0] = 0
                visual_img[i, j, 1] = 0
                visual_img[i, j, 2] = 255
            elif pixel_pred < 125 and pixel_true > 125:  # fn
                visual_img[i, j, 0] = 0
                visual_img[i, j, 1] = 98
                visual_img[i, j, 2] = 255
            elif pixel_pred < 125 and pixel_true < 125:  # tn
                visual_img[i, j, 0] = 0
                visual_img[i, j, 1] = 255
                visual_img[i, j, 2] = 0

    cv2.imwrite(os.path.join(save_dir, name), visual_img)
    #cv2.imwrite(os.path.join(save_dir, name.replace('.jpg', '_edge.jpg')), edge)
...
def plot_masks(
    ...
    for i in range(bs):
        ...
        for j in range(nc):
            r, g, b = colors[j]
            mask_bgr[mask[:,:, j] > 125] = np.array([r,g,b]).astype(np.uint8)
    ...
    if scale < 1:
        ...
        w = math.ceil(scale * w)
        mosaic = cv2.resize(mosaic, tuple(int(x * ns) for x in (w, h)))
        mask_mosaic = cv2.resize(mask_mosaic, tuple(int(x * ns) for x in (w, h)))
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("exp/detect/exp")):
    """
    ...
    Args:
        ...
        save_dir (Path, optional): Directory to save results. Defaults to Path('exp/detect/exp').
    """
...
def plot_masks(
    ...
    images_np = images.cpu().float().numpy() if isinstance(images, torch.Tensor) else images.clone()
    masks_np = masks.cpu().numpy() if isinstance(masks, torch.Tensor) else masks.clone()

    bs, _, h, w = images.shape  # batch size, _, height, width
    bs = min(bs, max_subplots)  # limit plot images
    ns = np.ceil(bs**0.5)  # number of subplots (square)
    if np.max(images_np[0]) <= 1:
        images_np =images_np * 255  # de-normalise (optional)

    if np.max(masks_np[0]) <= 1:
    ...
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        mosaic[y : y + h, x : x + w, :] = images_np[i].transpose(1, 2, 0)
        mask_bgr = np.ones((h, w, 3), dtype=np.uint8) * 255
        for j in range(nc):
            r, g, b = (255, 0, 0)
            mask_bgr[masks_np[i, 0] > 125, :] = np.array([b, g, r]).astype(np.uint8)
