...
Commands:
    ...
    3. To start a local MLflow server:
        mlflow server --backend-store-uri exp/mlflow
...
def on_pretrain_routine_end(trainer):
    ...
    Environment Variables:
        MLFLOW_TRACKING_URI: The URI for MLflow tracking. If not set, defaults to 'exp/mlflow'.
...
def on_train_end(trainer):
    ...
    if not mlflow:
        return
    mlflow.log_artifact(str(trainer.best.parent))  # log save_dir/weights directory with yolov11n-sky-seg.pt and last.pt
